...
For mortality, in which alive events are much more frequent than dead events, we avoid overestimating OOB by balancing the sampling weights to have the same number of observations from each class as suggested by Janitza & Hornung ([2018](https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0201904)).
...
dfImportance <- as.data.frame(matrix(rep(NA, length(var1) * nrow(dfPw)), ncol = length(var1)))
names(dfImportance) <- var1
    ...
    for(var in variables) {
      for(tree in nbTrees) {
        for(mty in nbMtry) {
          ...
          cat('   loading simulations', floor(count/nrow(dfPw) * 100), '%\r')
          count <- count + 1
        }
      }
    }
