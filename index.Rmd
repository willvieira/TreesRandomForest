...
The `r length(variables)` sets of variables for **growth** and **mortaltiy** are the following:
...
And for **recruitment**, in which is quantified at the $plot$ and $species$ level, the `r length(variables)`
sets of variables are:

```{r table of variables fec, echo=FALSE}
# get variables
for(var in variables) {
  v <- read.table(paste0('data/variables_fec', var, '.txt'))[, 1]
  v <- gsub("_mLag.*", "", v)
  assign(paste0('varFec', var), v)
}
for(var in variables) {print(paste('set', var)); print(as.character(get(paste0('varFec', var)))); cat('\n')}
```
...
dfPw <- expand.grid(c('fec', 'mort'), sp_ids, variables, nbTrees, nbMtry)
    ...
    for(var in variables) {
      for(tree in nbTrees) {
        for(mty in nbMtry) {
          ...
          # load simulation
          sim <- readRDS(paste0('output/', ifelse(vital != 'mort', vital, 'mort2'), '_', sp, '_var', var, '_nTrees', nbTrees, '_Mtry', mty, '.RDS'))
...
dfLong = gather(cbind(dfPw, dfImportance), variable, importance, growth:BA_sp, factor_key = TRUE)
...
p = ggplot(data = dfPw) +
    ...
    geom_boxplot() +
    facet_grid(~vital,
              labeller = labeller(vital = setNames(c('fec (Rsquared)',
...
p = ggplot(data = droplevels(subset(dfLong, var == 1))) +
...
Now for the other two sets of variables (`var2` and `var3`):

```{r variable set importance2, echo=FALSE}
p = ggplot(data = droplevels(subset(dfLong, var == 2 & variable %in% unique(c(as.character(var2), as.character(varFec2)))))) +
    aes(reorder(variable, importance, FUN = median), log(importance)) +
    geom_boxplot() +
    coord_flip() +
    facet_grid(~vital, scales = "free_x") +
...
p = ggplot(data = droplevels(subset(dfLong, var == 3 & variable %in% unique(c(as.character(var3), as.character(varFec3)))))) +
    aes(reorder(variable, importance, FUN = median), log(importance)) +
    geom_boxplot() +
    coord_flip() +
    facet_grid(~vital, scales = "free_x") +
...
p = ggplot(data = droplevels(subset(dfLong, var == 1 & variable %in% unique(c(as.character(var1), as.character(varFec1)))))) +
